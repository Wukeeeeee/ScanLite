...
<p align="center">  
  <a href="../../releases/latest"><img src="docs/version.svg" height="22" alt="当前版本 v1.3"></a>  
</p>

<p align="center">极简 Android 二维码 / 条形码扫描器 。 只申请相机与基础权限，不联网、无广告</p>
...
- **按「交给谁」分流**，而不是一律丢浏览器：
  - 普通网址 → 系统默认浏览器（或应用内选择已安装浏览器），**也可选「用微信打开」直接在微信内置浏览器渲染**
  - Wi-Fi 二维码 → **一键连接 Wi-Fi**（支持 Android 8.0 ~ 15+ 调起系统原生连接对话框/自动连入）
  - 微信专属码 / 微信支付码 → 自动复制内容 + 拉起微信（提示用户粘贴或扫一扫）
  - 支付宝码 → 直接进支付宝扫一扫/自解析接口
  - 淘宝 / 京东 / 拼多多 / 美团等专属链接 → 直接拉起对应 App，中途不会掉回浏览器
- **归属不唯一的码**会多给一行「也可以交给」，支付宝 / 微信自己选
- **「选择其他应用打开」是应用内列表**，不依赖系统选择器 —— 后者在部分国产 ROM 上会直接失灵
- 扫码历史、复制、分享
- **系统级优化**：ML Kit 解码器单例复用、历史记录 JSON 零冗余序列化、显式 GPU 硬件加速
- **零联网**：Manifest 里显式移除了 `INTERNET` / `ACCESS_NETWORK_STATE` 权限
- 锁竖屏，打开就是相机

## 下载安装

1. 到 [Releases](../../releases/latest) 下载 `ScanLite-1.3.apk`
...
- **普通网址支持微信内置浏览器直达，但微信原生「扫一扫」与「微信支付」无法直接唤起。**
  - 微信把二维码解码与支付接口组件（如 `GetQRCodeInfoUI`、`WXPayEntryActivity`）均设为了 `exported=false`，且 `wxp://` 等协议仅允许微信内部安全上下文处理。任何第三方 App 均无法直接触发微信原生扫码界面或直接唤起付款。
  - **ScanLite 的解决方案**：普通网页链接可以通过「用微信打开」直接在微信内置浏览器中加载；对于微信专属码/付款码，ScanLite 会自动将内容复制到剪贴板并拉起微信，方便用户直接粘贴或使用微信扫一扫。
